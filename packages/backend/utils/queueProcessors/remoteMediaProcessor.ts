...
async function processRemoteMedia(job: Job) {
  try {
    ...
    if (!media) return
    const mediaUrl = media.external ? media.url : completeEnvironment.mediaUrl + media.url
    let fileLocation = ''
    await getMediaFromUrl(mediaUrl, undefined, false, {
      priority: 2097151,
      parentData: { id: job.id as string, queue: 'processRemoteMediaData' }
    })
