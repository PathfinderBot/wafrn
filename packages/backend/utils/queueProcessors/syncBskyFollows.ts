...
async function syncBskyFollowsJob(job: Job) {
  ...
  if (user && user.bskyDid) {
    ...
    while (followersResponse.data.followers.length > 0) {
      ...
      followersPage++
      if (followersResponse.data.cursor && followersPage < MAX_SYNC_PAGES) {
        ...
        if (followersResponse.data.cursor) {
          logger.debug({
            message: `Aborting syncBskyFollowsJob followers sync: max page count exceeded`,
            userId,
            page: followersPage
          })
        }
    ...
    while (followingResponse.data.follows.length > 0) {
      ...
      followingPage++
      if (followingResponse.data.cursor && followingPage < MAX_SYNC_PAGES) {
        ...
        if (followingResponse.data.cursor) {
          logger.debug({
            message: `Aborting syncBskyFollowsJob following sync: max page count exceeded`,
            userId,
            page: followingPage
          })
        }
    ...
    const didsToForceFollow = wafrnOnlyFollowsToAddToBsky
      .map((elem) => elem.followed.bskyDid)
      .filter((elem) => !followingDids.includes(elem as string))
    if (user.email && didsToForceFollow.length > 0) {
      const userAgent = await getAtProtoSession(user, true)
      for await (const did of didsToForceFollow.filter((elem) => elem && elem.toLowerCase().startsWith('did'))) {
    ...
    if (pds) {
      ...
      let url =
        pds +
        `/xrpc/com.atproto.repo.listRecords?repo=${encodeURIComponent(user.bskyDid)}&collection=app.bsky.graph.block&limit=100&reverse=false`
      let blocksPage = 0
      while (remainingBlocks != 0) {
        if (blocksPage >= MAX_SYNC_PAGES) {
          logger.debug({
            message: `Aborting syncBskyFollowsJob blocks sync: max page count exceeded`,
            userId,
            page: blocksPage
          })
          break
        ...
        blocksPage++
        // SECURITY (11.1): getServerFromDid already rejects internal/private PDS targets,
        // but we re-validate here (defense in depth) since this URL is built and fetched
        // independently, and is rebuilt every iteration with a fresh cursor.
        await assertUrlResolvesPublic(url)
        const blocksResponse = await (
          await fetch(url, {
            headers: {
        ...
        url =
          pds +
          `/xrpc/com.atproto.repo.listRecords?repo=${encodeURIComponent(user.bskyDid)}&collection=app.bsky.graph.block&limit=100&reverse=false&cursor=${blocksResponse.cursor}`
