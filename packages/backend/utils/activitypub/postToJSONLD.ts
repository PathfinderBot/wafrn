...
import { Media, Post, Quotes, sequelize, User } from '../../models/index.js'
...
async function postToJSONLD(
  ...
  const stringMyFollowing = `${completeEnvironment.frontendUrl}/fediverse/blog/${localUser.url.toLowerCase()}/following`
  ...
  WITH RECURSIVE ancestors AS (
    SELECT id, "parentId" FROM posts WHERE id = :postParentId
    UNION ALL
    SELECT p.id, p."parentId" FROM posts p
    INNER JOIN ancestors a ON p.id = a."parentId"
  )
  SELECT "parentId" as "ancestorId" FROM ancestors WHERE "parentId" IS NOT NULL
  `,
      {
    ...
    let ancestors: Post[] = []
    const ancestorIds: string[] = ancestorIdsQuery.map((elem) => elem.ancestorId)
  ...
  if (ask) {
    ...
    misskeyAskContent = `$[border.style=solid,width=1,radius=6 $[border.color=0000,width=12 ${getUserName(userAsker)} [asked](${
      completeEnvironment.frontendUrl + '/fediverse/post/' + post.id
    }):
${(await htmlToMfm(ask.question)).replaceAll('[', '').replaceAll(']', '')}]]\n\n`
  ...
  for await (const tag of post.postTags) {
    ...
    misskeyTagsAndQuotes = `${misskeyTagsAndQuotes} ${
      tag.tagName.trim().includes(' ') ? '# ' + tag.tagName.trim() : '#' + tag.tagName.trim()
    }`
    fediTags.push({
      type: 'Hashtag',
  ...
  for await (const userId of mentions) {
    ...
    if (!misskeyContent.includes(user.url) && !misskeyAskContent.includes(user.url)) {
  ...
  misskeyContent = await htmlToMfm(misskeyContent.replace(lineBreaksAtEndRegex, ''))
  if (misskeyTagsAndQuotes.length > 0) {
    misskeyContent = misskeyContent + `\n<small>${await htmlToMfm(misskeyTagsAndQuotes)}</small>`
  }
  ...
  const misskeyMarkdown = misskeyMentionContent + misskeyAskContent + misskeyContent
  ...
  const initialMentionsToRemoveTag: fediverseTag[] =
    standardMentions.length > 0
      ? [
          {
  ...
  let postAsJSONLD: activityPubObject = {
    ...
    object: {
      ...
      forceDescendentsToUseSameInteractionControls:
        post.hierarchyLevel === 1 && post.replyControl != InteractionControl.Anyone ? true : undefined,
      interactionPolicy: {
        canQuote: {
          automaticApproval:
            post.quoteControl === InteractionControl.Anyone ? ['https://www.w3.org/ns/activitystreams#Public'] : []
        },
        canLike: {
