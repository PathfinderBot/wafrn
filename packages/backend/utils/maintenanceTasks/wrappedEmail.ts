...
import { EmojiReaction, Follows, Notification, Post, PostAncestor, User } from '../../models/index.js'
...
async function sendMail() {
  ...
  for await (const user of users) {
    ...
    const posts = parseInt(currentUserCounts.postCount)
    const rewoots = parseInt(currentUserCounts.reblogCount)
    ...
    const postQuotesRewoots2 =
      Object.groupBy(postQuotesRewoots, ({ id }) => postQuotesRewootsAncestors.find(x => x.postsId === id)?.ancestorId ?? '')
    ...
    const mostRewootedPosts = postQuotesRewoots3.sort((a, b) => b.rewoots - a.rewoots)
    const mostQuotedPosts = postQuotesRewoots3.sort((a, b) => b.quotes - a.quotes)
    ...
    const mostReactedPosts = (await Post.findAll({
      where: {
        id: {
    ...
    const mostRepliedPosts = ((await Post.findAll({
      attributes: [
        'parentId',
        [fn('COUNT', col('parentId')), 'replyCount']
      ],
      where: {
        parentId: {
          [Op.in]: allUserPosts.map(x => x.id)
      ...
      group: ['parentId'],
      raw: true
    }) as any) as { parentId: string, replyCount: string }[]).map(x => ({
      ...x,
    ...
    <p>You wooted ${posts} woots on this year, that's ${calcPercentile(posts, allUserCounts.map(x => x.postCount))}% more than others!</p>
    <p>Also you rewooted ${rewoots} woots on this year, that's ${calcPercentile(rewoots, allUserCounts.map(x => x.reblogCount))}% more than others!</p>
    <p>You got followed by ${yearFollowers.count} people on this year, and you followed ${yearFollows.count} people on this year!</p>
    <br />
    <p>Now let's go to the juicy parts</p>
    ${mostRewootedPosts[0] ? `<p>The most rewooted woot you have is ${new URL(`/fediverse/post/${mostRewootedPosts[0].parentId}`, completeEnvironment.frontendUrl)} which has ${mostRewootedPosts[0].rewoots} rewoots</p>` : ''}
    ${mostQuotedPosts[0] ? `<p>The most quoted woot you have is ${new URL(`/fediverse/post/${mostQuotedPosts[0].parentId}`, completeEnvironment.frontendUrl)} which has ${mostQuotedPosts[0].quotes} quotes</p>` : ''}
...
function calcPercentile(userVal: string | number, allVals: string[] | number[]): string {
