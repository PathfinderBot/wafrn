...
import { getKey } from '../utils/cacheGetters/getKey.js'
...
function getCheckFediverseSignatureFunction(force = false) {
  return async function checkFediverseSignature(req: SignedRequest, res: Response, next: NextFunction) {
    if (
      req.headers['Accept'] &&
      (req.headers['Accept'] as string).toLowerCase().includes('application/activity+yaml')
    ) {
      res.status(406)
      return res.send({
        error: 'We do not accept yaml-ld',
        message:
          'YAML-LD will generate a lot of safety issues: https://pyyaml.org/wiki/PyYAMLDocumentation#:~:text=Warning,though '
      })
    }
    let success = !force
    ...
    try {
      ...
      if (remoteKeyData.key) {
        ...
        if (
          ...
          const tmpUser = await getRemoteActor(remoteUserUrl, adminUser as User)
      ...
      if (req.method === 'POST') {
        if (verifyDigest(req.rawBody ? req.rawBody : '', req.headers.digest)) {
          if (
            ...
            // Track *who owns the key we are about to verify against*, independent of the object
            // shape returned below, so we always have an identity to bind `actor` to.
            let signerIdentity: string | undefined = remoteKeyData.key ? remoteUserUrl : undefined
            const remoteActor = remoteKeyData.key
              ? { publicKey: remoteKeyData.key }
              : await getRemoteActor(signature.creator.split('#')[0], adminUser as User)
            if (!remoteKeyData.key && remoteActor) {
              signerIdentity =
                (remoteActor as any).remoteId || (remoteActor as any).url || signature.creator.split('#')[0]
            }
            ...
            const actorBoundToSigner =
              typeof req.body.actor === 'string' &&
              typeof signerIdentity === 'string' &&
              signerIdentity.toLowerCase() === req.body.actor.toLowerCase()

            if (!actorBoundToSigner) {
              logger.debug({
...
function checkForForbiddenDirectives(value: any) {
  const forbiddenDirectives = ['@included', '@graph', '@reverse']
