...
export const baseEnvironment: Environment = {
  ...
  logSQLQueries: process.env.LOG_SQL_QUERIES
  ...
  workers: {
    ...
    mainThread: process.env.USE_WORKERS
  ...
  externalCacheBackups: process.env.FRONTEND_CACHE_BACKUP_URLS
    ? [process.env.FRONTEND_CACHE_BACKUP_URLS ?? ""]
    : [],
  ...
  reviewRegistrations: process.env.REVIEW_REGISTRATIONS
    ? process.env.REVIEW_REGISTRATIONS == "true"
    : true,
  ...
  ignoreBlockHosts: process.env.IGNORE_BLOCK_HOSTS
    ? process.env.IGNORE_BLOCK_HOSTS.split(",")
    : [],
  // default SEO data that will be used when trying to load server data
  defaultSEOData: {
    title:
      process.env.FRONTEND_SHORT_TITLE ??
  ...
  autoFollowAdmin: process.env.AUTOFOLLOW_MAIN_ADMIN
    ? process.env.AUTOFOLLOW_MAIN_ADMIN == "true"
    : true,
  minimumAgeToRegister: parseInt(process.env.REGISTRATION_MINIMUM_AGE ?? "18"),
  registrationLevel: (process.env.REGISTRATION_LEVEL ?? "PUBLIC") as
    | "PRIVATE"
    | "INVITE"
    | "PUBLIC",
  disableShowingBlockedServers: process.env.HIDE_BLOCKED_SERVERS == "true",
  bubbleHostsShowType: (process.env.BUBBLE_SERVERS_SHOW_TYPE ?? "PUBLIC") as
    | "PUBLIC"
    | "HIDDEN"
    | "LOGGEDIN",
  blockedHostsShowType: (process.env.BLOCKED_SERVERS_SHOW_TYPE ?? "LOGGEDIN") as
    | "PUBLIC"
    | "HIDDEN"
    | "LOGGEDIN",
  ...
  frontendEnvironment: {
    ...
    minimumAgeToRegister: parseInt(
      process.env.REGISTRATION_MINIMUM_AGE ?? "18",
    ),
    donationUrl: process.env.DONATION_URL ?? "",
    disableShowingBlockedServers: process.env.HIDE_BLOCKED_SERVERS == "true",
