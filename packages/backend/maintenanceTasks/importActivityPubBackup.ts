...
import { CreateActivity } from '../queueProcessors/processors/create.js'
import { Post, User, UserBookmarkedPosts, UserLikesPostRelations } from '../models/index.js'
import { AnnounceActivity } from '../queueProcessors/processors/announce.js'
...
async function importBackup(fileName: string, userUrl: string) {
  ...
  for (const entry of Object.values(await zip.entries())) {
    if (entry.name == 'outbox.json') {
      backupData = JSON.parse((await zip.entryData(entry)).toString('utf8').replaceAll('\\u0000', ''))
    } else if (entry.name == 'likes.json') {
      likeData = JSON.parse((await zip.entryData(entry)).toString('utf8').replaceAll('\\u0000', ''))
    } else if (entry.name == 'bookmarks.json') {
      bookmarkData = JSON.parse((await zip.entryData(entry)).toString('utf8').replaceAll('\\u0000', ''))
    } else if (entry.name == 'actor.json') {
      actorData = JSON.parse((await zip.entryData(entry)).toString('utf8').replaceAll('\\u0000', ''))
    ...
    if (backupData?.orderedItems) {
      for (const item of backupData.orderedItems) {
        ...
        if (item.type.toLowerCase() === 'create') {
          try {
            const post = await getPostThreadRecursive(user, item.object.id, item.object, undefined, {
              allowMediaFromBanned: true
            })
            if (post) {
