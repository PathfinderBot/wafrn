...
export default function statusRoutes(app: Application) {
  ...
  app.get('/api/status/bubble', optionalAuthentication, async (req: AuthorizedRequest, res: Response) => {
    if (
  ...
  app.get('/api/status/blocks', optionalAuthentication, async (req: AuthorizedRequest, res: Response) => {
    if (
