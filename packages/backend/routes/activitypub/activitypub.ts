...
function activityPubRoutes(app: Application) {
        ...
        if (user && !user.banned) {
          ...
          if (req.query?.page) {
            ...
            response = {
              ...
              }/fediverse/blog/${user.url.toLowerCase()}/following?page=${page}`,
              type: "OrderedCollectionPage",
              partOf: `${
                completeEnvironment.frontendUrl
              }/fediverse/blog/${user.url.toLowerCase()}/following`,
              orderedItems: itemsToSend,
            };
            if (page > 1) {
              response["prev"] = `${
                completeEnvironment.frontendUrl
              }/fediverse/blog/${user.url.toLowerCase()}/following?page=${
                page - 1
              }`;
            }
            ...
            response = {
              ...
              first: `${
                completeEnvironment.frontendUrl
              }/fediverse/blog/${user.url.toLowerCase()}/following?page=1`,
            };
          }
          res.set({
            "content-type": "application/activity+json",
        ...
        if (user && !user.banned) {
          ...
          if (req.query?.page) {
            ...
            response = {
              ...
              id: `${
                completeEnvironment.frontendUrl
              }/fediverse/blog/${user.url.toLowerCase()}/followers?page=${
                req.query.page
              }`,
              type: "OrderedCollectionPage",
              orderedItems: itemsToSend,
            ...
            response = {
              ...
              first: `${
                completeEnvironment.frontendUrl
              }/fediverse/blog/${user.url.toLowerCase()}/followers?page=1`,
            };
          }
          res.set({
            "content-type": "application/activity+json",
  ...
  app.post(
    ...
    async (req: SignedRequest, res: Response) => {
      const urlToSearch = req.params?.url
        ? req.params.url
        : completeEnvironment.adminUser;
      const url = urlToSearch.toLowerCase();
      const user = await getLocalUserByUrlCache(url);
      if (
        user &&
        user.url !== completeEnvironment.adminUser &&
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      res.send({
        type: "CollectionPage",
        partOf: `${completeEnvironment.frontendUrl}/fediverse/post/${
