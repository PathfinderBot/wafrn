...
export const up: Migration = async (params) => {
  if (completeEnvironment.enableBsky) {
    ...
    const repliedPosts: string[] = (
      await queryInterface.sequelize.query(
        `SELECT "bskyUri" FROM "posts" WHERE "id" IN (SELECT "ancestorId" FROM "postsancestors" WHERE "postsId" IN (SELECT "id" FROM  "posts" where "userId" IN (select "id" FROM "users" WHERE "email" IS NOT NULL) AND "hierarchyLevel"!=1 AND "bskyUri" IS NOT NULL ORDER BY "createdAt" DESC LIMIT 10000)) and "hierarchyLevel" = 1 and "bskyUri" is NOT NULL`
      )
