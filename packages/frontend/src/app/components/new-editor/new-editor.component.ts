...
export class NewEditorComponent implements OnInit, OnDestroy {
  ...
  currentUser: Signal<BlogDetails | undefined> = computed(
    () => this.accountList().at(this.posterAccount())?.blog
  );
  ...
  constructor(
    ...
    if (this.data?.post?.mentionPost && this.data.post.mentionPost.length > 0) {
      const mentionedUsersSet = new Set(
        this.data.post.mentionPost.filter((elem) => elem.id != currentUserId)
      );
    ...
    if (this.data?.post) {
      if (
        ...
        !this.mentionedUsers
          .map((elem) => elem.id)
          .includes(this.data.post.user.url)
      ) {
    ...
    if (this.editing && this.data?.post) {
      if (this.data.post.markdownContent) {
        this.postCreatorForm.controls["content"].patchValue(
          this.data.post.markdownContent
        );
      } else {
        this.messages.add({
          severity: "warn",
    ...
    if (this.editorUpdatedSubscription) {
      ...
      this.cursorPosition = {
        ...
        y: Math.min(
          ...
          Math.max(
            screenHeight - 325,
            screenHeight - 64 * this.suggestions.length
          )
        ),
      };
    }
  ...
  updateMentionsSuggestions(newText: string | null, cursorPosition: number) {
    ...
    const textToMatch = (" " +
      newText
        ?.slice(cursorPosition - 250, cursorPosition)
        .replaceAll("\n", " ")) as string;
    ...
    if (matchIsUser) {
      ...
      const matchedEmojiIds = [
        ...new Set(
          this.fuse
            .search(match)
            .map((res) => res.item.id.replaceAll(/([\uFE00-\uFE0F])/g, ""))
        ),
      ].slice(0, 25);
  ...
  insertMention(user: { img: string; text: string }) {
    let initialPart = (" " +
      this.postCreatorForm.value.content?.slice(
        0,
    ...
    let finalPart = this.postCreatorForm.value.content?.slice(
      this.cursorTextPosition
    ) as string;
    this.postCreatorForm.controls["content"].patchValue(
      initialPart.trim() + " " + finalPart.trim()
    );
  ...
  getPrivacyIcon() {
    ...
    const res = this.privacyOptions.find((elem) => elem.level === this.privacy)
  ...
  async submitPost() {
    ...
    let res = undefined;
    const content = this.postCreatorForm.value.content
  ...
  editorFocusedIn() {
    this.editorUpdatedSubscription = this.postCreatorForm.controls[
      "content"
    ].valueChanges
      .pipe(debounceTime(500))
  ...
  editorUpdateProcess(change: string | null) {
    ...
    const textSelected =
      postElement.selectionStart !== postElement.selectionEnd;
  ...
  calculateBskyPostLength() {
    const cwText =
      this.contentWarning.length > 0 ? `[${this.contentWarning.trim()}]\n` : "";
    const tagText =
      this.tags.length > 0
        ? `\n${this.tags
            .split(",")
  ...
  handlePaste(event: ClipboardEvent) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const itemIsMedia = mediaFormats.some((format) =>
        element.type.includes(format)
      );
      if (element.type === "application/x-moz-file") {
        this.messages.add({
          severity: "warn",
          summary:
            "Firefox does not allow reading the clipboard from file explorers, try dragging the file in",
        });
      }
      if (itemIsMedia) {
  ...
  handleDrop(event: DragEvent) {
    ...
    if (event.dataTransfer) {
      for (let i = 0; i < event.dataTransfer.files.length; i++) {
        ...
        const mediaFormats = ["image", "video", "audio"];
        const fileIsMedia = mediaFormats.some((format) =>
