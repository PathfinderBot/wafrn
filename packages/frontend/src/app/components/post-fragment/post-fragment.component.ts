...
export class PostFragmentComponent implements OnChanges, OnDestroy {
  ...
  postHasLinkPreview = computed<boolean>(
    () =>
      this.fragment().medias.find(
        (media) => media.mediaType === "text/html"
      ) !== undefined
  );

  readonly inlineMediaElement =
    viewChild<ElementRef<HTMLElement>>("mediaInline");
  ...
  ngOnInit(): void {
    ...
    this.likeSubscription = this.postService.postLiked.subscribe(
      (likeEvent) => {
    ...
    this.emojiSubscription = this.postService.emojiReacted.subscribe(
      (emojiEvent) => {
    ...
    let content = this.postService
      .getPostHtml(this.fragment(), [])
      .toLowerCase();
    ...
    if (mentions) {
      this.mentionPosts = mentions
        .filter((usr) => {
          ...
          return (
            !content.includes(userUrl) && usr.url != this.fragment().user.url
          );
        })
        .map((user) => user.url);
    }
  ...
  ngOnChanges(): void {
    ...
    this.nonLinkMediaCount = this.fragment().medias.filter(
      (elem) => elem.mediaType != "text/html"
    ).length;
  }
  ...
  processEmojis() {
    for (let emoji of this.emojiCollection()) {
      emoji.tooltip =
        (this.isLike(emoji.content) ? "Liked" : emoji.content) +
        " by " +
    ...
    this.emojiCollection.set(
      this.emojiCollection()
        .sort(
          (a, b) =>
            +(this.availableEmojiNames.has(b.name) || !b.img) -
            +(this.availableEmojiNames.has(a.name) || !a.img)
        )
  ...
  async toggleEmojiReact(emojiReaction: EmojiReaction, event: MouseEvent) {
    ...
    if (this.isLike(emojiReaction.content)) {
      ...
      let response = false;
      if (reactionIsToggled) {
        response = await this.postService.emojiReactPost(
          postId,
          emojiReaction.content,
          true
        );
        if (response) {
          this.messages.add({
  ...
  async cwClick() {
    ...
    if (
      ...
      this.settingsService.values().confirmOpenCw
    ) {
      ...
      const confirm = await this.simpleDialog.createConfirmDialog({
        ...
        contentSuffix:
          ...
          (this.fragment().muted_words_cw !== undefined
            ? `; ${this.fragment().muted_words_cw}`
            : ""),
        annoying: annoyance,
      });
