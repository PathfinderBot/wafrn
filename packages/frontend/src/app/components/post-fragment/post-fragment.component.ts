...
export class PostFragmentComponent implements OnChanges, OnDestroy {
  ...
  wafrnFormattedContent = computed(() => {
    ...
    if (localStorage.getItem('replaceAIWithCocaine') === 'true') {
      ...
      let regexpString = wordsToReplace.map((elem) => `\\s${elem}\\s|^${elem}\\s|\\s${elem}$`).join('|')
  ...
  initializeEmojis() {
    ...
    this.fragment().emojiReactions.forEach((reaction) => {
      ...
      if (
        ...
        !emojiReactions[reaction.content].users.some((user) => user.id === reaction.user?.id)
      ) {
  ...
  renderLikeDislike({ like }: { id: string; like: boolean }) {
    if (like) {
      const alreadyLiked = this.fragment().emojiReactions.some(
        (reaction) => reaction.userId === this.userId && reaction.emojiId === 'Like' && this.isLike(reaction.content)
      )
      if (!alreadyLiked) {
  ...
  renderEmojiReact({ emoji, type }: { postId: string; emoji: Emoji; type: 'react' | 'undo_react' }) {
    ...
    if (type === 'react') {
      const alreadyReacted = this.fragment().emojiReactions.some(
        (reaction) => reaction.userId === this.userId && reaction.content === emoji.name
      )
      if (!alreadyReacted) {
        this.fragment().emojiReactions.push({
          emojiId: emoji.id,
