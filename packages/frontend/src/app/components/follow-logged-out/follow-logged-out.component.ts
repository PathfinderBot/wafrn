  ...
  constructor() {
    ...
    this.remoteId =
      (!data.remoteId?.startsWith(EnvironmentService.environment.frontUrl) && this.url.startsWith('@')) ?
        data.remoteId : (!this.url.replace(EnvironmentService.environment.frontUrl + '/blog/', '').startsWith('@')) ? this.url : undefined
  }

  async onConfirm() {
    this.disabled = true
    const fediHost = (!!this.fediUsername().replace(/^@/, '').split('@')[1]) ? this.fediUsername().replace(/^@/, '').split('@')[1] : this.fediUsername()
    const webfinger = await fetch(`${!fediHost.startsWith('http') ? 'https://' : ''}${fediHost}/.well-known/webfinger?resource=${encodeURIComponent(
