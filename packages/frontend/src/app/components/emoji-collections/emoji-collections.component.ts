...
export class EmojiCollectionsComponent implements AfterViewInit, OnDestroy {
  ...
  vcRows = computed<VirtualRows>(() => {
    ...
    if (
      this.includedCollectionsSize() == 0 ||
      this.includedCollections.has(-2)
    ) {
    ...
    for (let i = 0; i < this.emojiCollections.length; i++) {
      ...
      let newCollection: EmojiCollection = {
        name: c.name,
        comment: c.comment,
        emojis: [],
      };
      if (
        this.includedCollectionsSize() == 0 ||
        this.includedCollections.has(i)
      ) {
  ...
  collectionHas(index: number): boolean {
    ...
    return (
      this.includedCollections.has(index) && this.includedCollections.size != 0
    );
  }

  showCollection(collection: EmojiCollection): boolean {
    return collection.emojis
      .map((elem) => elem.name)
      .some((elem) => elem.includes(this.filterText()));
  }

  updateDimensions() {
    if (this.emojiElement) {
      const emojiFreeWidth =
        this.emojiElement.nativeElement.offsetWidth - 2 * this.rowMargin;
      this.emojiPerRow.set(
        Math.max(Math.floor(emojiFreeWidth / this.emojiWidth) - 1, 1)
      );
    }
      ...
      next: () => {
        if (!skipNext) {
          ...
          if (
            ...
            (collection == -1
              ? this.recents.length
              : this.collections[collection].emojis.length)
          ) {
            ...
            value: {
              ...
              name:
                collection == -1
                  ? "Recent emojis"
                  : this.collections[
                      Math.min(collection, this.collections.length - 1)
                    ]?.name,
              index: 0,
              array: [],
              count:
