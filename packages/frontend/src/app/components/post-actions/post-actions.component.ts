...
export class PostActionsComponent implements OnChanges {
  ...
  ngOnChanges(): void {
    this.myRewootsIncludePost = this.postService
      .rewootedPosts()
      .has(this.post().id);
  ...
  async silencePost(superMute: boolean = false) {
    const confirm = await this.simpleDialog.createConfirmDialog({
      title: !superMute
        ? "dialog.post-header.silenceInteractionsTitle"
        : "dialog.post-header.silenceReplyTitle",
      content: !superMute
    ...
    const success = await this.postService.silencePost(
      this.post().id,
      superMute
    );
  ...
  private async checkPostSilenced() {
    this.postSilenced = (await this.utilsService.getSilencedPostIds()).includes(
      this.post().id
    );
  }
