...
import {
  ...
  SnappyHide,
  SnappyShow,
} from "src/app/components/snappy/snappy-life";
  ...
  implements OnInit, OnDestroy, SnappyCreate, SnappyShow, SnappyHide
{
  ...
  scroll = 0;
  hideQuotesLevel = localStorage.getItem("hideQuotes")
  ...
  ngOnInit(): void {
    ...
    this.updateFollowersSubscription =
      this.postService.updateFollowers.subscribe(() => {
        if (
          this.postService.followedUserIds.length <= 1 &&
          this.level === 1 &&
          false
        ) {
  ...
  async loadPosts(page: number) {
      ...
      .filter((post: ProcessedPost[]) => {
          ...
          if (
            post.some((elem) => {
              !this.postService.followedUserIds.includes(elem.userId) ||
                (this.postService.usersQuotesDisabled.includes(elem.userId) &&
                  elem.quotes &&
                  elem.quotes.length);
            })
          ) {
        ...
        if (
          (superMutedWords.length > 0 &&
            superMutedWords.some((supermuteWord) =>
              textOfPosts.includes(supermuteWord)
            )) ||
          (!localStorage.getItem(
            ...
            post.some(
              (elem) =>
                (
                  elem.mentionPost?.filter((mention) =>
                    this.postService.blockedUserIds.includes(mention.id)
                  ) || []
                ).length > 0
            ))
        ) {
    ...
    if (!this.jwtService.tokenValid()) {
      const welcomePost = await this.dashboardService.getArticle(
        "system.welcome"
      );
