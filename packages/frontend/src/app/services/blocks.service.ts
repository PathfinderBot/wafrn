...
export class BlocksService {
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.http.post<{ success: boolean }>(
          `${EnvironmentService.environment.baseUrl}/block`,
          formData
        )
      );
      if (response?.success) {
  ...
  processResponse(serverResponse: Array<any>, key: string): UserBlockMute[] {
    return serverResponse.map((userBlocked) => {
      return {
        avatar: `${EnvironmentService.environment.cacheDomain}/api/v2/cache/avatar/${userBlocked[key].id}`,
  ...
  async getBlockList(): Promise<UserBlockMute[]> {
    const response = await firstValueFrom(
      this.http.get<Array<any>>(
        `${EnvironmentService.environment.baseUrl}/myBlocks`
      )
    );
  ...
  async unblockUser(id: string): Promise<UserBlockMute[] | undefined> {
    const response = await firstValueFrom(
      this.http.post<Array<any>>(
        ...
        }/unblock-user?id=${encodeURIComponent(id)}`,
        {}
      )
    );
    if (response) {
  ...
  async unmuteUser(id: string): Promise<Array<any>> {
    const response = await this.http
      .post<Array<any>>(
        `${
          EnvironmentService.environment.baseUrl
        }/unmute-user?id=${encodeURIComponent(id)}`,
        {}
      )
      .toPromise();
    if (response) {
  ...
  async blockServer(id: string): Promise<boolean> {
    ...
    try {
      ...
      const response = await this.http
        .post(
          `${EnvironmentService.environment.baseUrl}/blockUserServer`,
          formData
        )
        .toPromise();
      this.messages.add({
  ...
  async getMyServerBlockList(): Promise<Array<any>> {
    const response = await this.http
      .get<Array<any>>(
        `${EnvironmentService.environment.baseUrl}/myServerBlocks`
      )
      .toPromise();
  ...
  async unblockServer(id: string): Promise<Array<any>> {
    const response = await this.http
      .post<Array<any>>(
        `${
          EnvironmentService.environment.baseUrl
        }/unblockServer?id=${encodeURIComponent(id)}`,
        {}
      )
      .toPromise();
    return response ? this.processResponseServer(response) : [];
