...
export class FallbackTranslateLoader implements TranslateLoader {
  ...
  getTranslation(lang: string): Observable<any> {
    ...
    if (lang === 'en') {
      return this.http.get(`${this.prefix}en${this.suffix}`).pipe(
        catchError(() => of({}))
      );
    }
    ...
    return forkJoin({
      langTranslations: this.http.get(`${this.prefix}${lang}${this.suffix}`).pipe(
        catchError(() => of({}))
      ),
      englishTranslations: this.http.get(`${this.prefix}en${this.suffix}`).pipe(
        catchError(() => of({}))
      )
    }).pipe(
