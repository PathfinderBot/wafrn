...
if [ -z "${ADMIN_PASSWORD}" ]; then
  echo creating ADMIN_PASSWORD
  export ADMIN_PASSWORD="$(openssl rand -base64 24 | tr '+/' '_-')"
fi

if [ -z "${GF_SECURITY_ADMIN_PASSWORD}" ]; then
  echo creating GF_SECURITY_ADMIN_PASSWORD
...
echo "Done! All env variables saved to .env"
